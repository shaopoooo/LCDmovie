...
void InitialLCD(void)
{
	...
	delay1(300000);
	WriteIns(LCD_DisplayCursor | LCD_Display);  // display on D=C=1 entire display + cursor on  B=0 cursor position off
	delay1(300000);
	WriteIns(LCD_EntryMode | LCD_ACShiftDirection);  // set input mode   I/D=1 increment  S=0  shift off
	delay1(300000);
}

void ball(int line,int loc ,int delay)
{
	int i,j;
	int animate[6][8]={
		{0x00,0x10,0x18,0x14,0x0b,0x04,0x08,0x0a},  //左上
		{0x00,0x01,0x03,0x05,0x1a,0x04,0x02,0x0a},  //右上
		{0x08,0x07,0x04,0x0a,0x09,0x08,0x07,0x02},  //左下
		{0x02,0x1c,0x04,0x0a,0x12,0x02,0x1c,0x08},  //右下
		{0x03,0x04,0x08,0x09,0x09,0x08,0x04,0x03},
		{0x18,0x04,0x02,0x12,0x12,0x02,0x04,0x18}
	};

	for(j=0;j<6;j++){
		for(i=0;i<8;i++){
			 WriteIns(LCD_SetCGRAM|8*j|i );//LCD_SetCGRAM function
			 WriteData(animate[j][i]);
		}
	}
		WriteIns(LCD_ClearDisplay );
		SetCursor(line,13-loc);
		WriteData(4);
		SetCursor(line,14-loc);
		WriteData(5);
		delay1(delay);

}

void picaL(int delay)
{
	int i,j;
	int animate[4][8]={
		{0x00,0x10,0x18,0x14,0x0b,0x04,0x08,0x0a},  //左上
...
void boom1(int delay)
{
	int i,j;
	int animate[4][8]={
		{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01},  //左上
		{0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x10},  //右上
		{0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00},  //左下
		{0x10,0x00,0x00,0x00,0x00,0x00,0x00,0x00},  //右下
...
void boom2(int delay)
{
	int i,j;
	int animate[4][8]={
		{0x00,0x00,0x00,0x00,0x00,0x05,0x02,0x05},  //左上
		{0x00,0x00,0x00,0x00,0x00,0x04,0x08,0x14},  //右上
		{0x0b,0x02,0x04,0x00,0x00,0x00,0x00,0x00},  //左下
		{0x1a,0x08,0x14,0x00,0x00,0x00,0x00,0x00},  //右下
...
void boom3(int delay)
{
	int i,j;
	int animate[4][8]={
		{0x00,0x00,0x00,0x01,0x08,0x05,0x02,0x05},  //左上
		{0x00,0x00,0x10,0x00,0x12,0x04,0x08,0x15},  //右上
		{0x0b,0x02,0x04,0x09,0x00,0x01,0x00,0x00},  //左下
		{0x1a,0x08,0x14,0x02,0x10,0x00,0x00,0x00},  //右下
...
void picaMusic(int delay)
{
	...
	int animate[6][8]={
		...
		{0x08,0x07,0x04,0x0a,0x09,0x08,0x07,0x02},  //左下
		{0x02,0x1c,0x04,0x0a,0x12,0x02,0x1c,0x08},  //右下
		{0x06,0x05,0x05,0x06,0x1c,0x1c,0x1c,0x00},
		{0x04,0x06,0x05,0x05,0x06,0x1c,0x1c,0x1c}
	};
...
void picalight(int loc,int delay)
{
	int i,j;
	int animate[6][8]={
		{0x00,0x10,0x18,0x14,0x0b,0x04,0x08,0x0a},  //左上
		...
		{0x08,0x07,0x04,0x0a,0x09,0x08,0x07,0x02},  //左下
		{0x02,0x1c,0x04,0x0a,0x12,0x02,0x1c,0x08},  //右下
		{0x3,0x6,0xc,0x1f,0x6,0xc,0x18,0x00}
